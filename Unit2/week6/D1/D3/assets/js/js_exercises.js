...
*/
const firstPet = pets.shift(); // Rimuove il primo elemento dall'array e lo assegna a "firstPet"
pets.push(firstPet); // Aggiunge l'elemento "firstPet" in ultima posizione nell'array "pets"
console.log(pets); 
/* ESERCIZIO 6
...
const carsWithLicensePlate = cars.map(car => {
    return {
      ...car,
      licensePlate: 'AB123CD' // valore a mia scelta
    };
  });
  
console.log(carsWithLicensePlate);
//seconda prov//
cars.forEach(car => {
...
*/
function addCar(brand, model, color, trims,licensePlate) {
    const newCar = {
      brand,
      model,
      color,
  ...
  console.log(addCar('Tesla', 'Model 3', 'white', ['standard', 'long range', 'performance'],'AB123CD' ));
...
*/
const carsWithMoreThanTwoTrims = cars.filter(car => car.trims.length > 2);
console.log(carsWithMoreThanTwoTrims);
...
*/
const carBrandAndModel = cars.map(car => car.brand + ' ' + car.model);
...
console.log(numericArray[i]); // stampa anche il numero 32
